...
public class ViewFactory(IServiceProvider serviceProvider) : IViewFactory
{
        ...
        where T : class
    {
        ...
        if (typeof(T) == typeof(VariableGridView))
        {
            var rowCount = (int)args![0]!;
            var colCount = (int)args![1]!;
            var useCellName = args.ElementAtOrDefault(2) as bool? ?? true;
            var headerColumns = args.ElementAtOrDefault(3) as string[];
            var headerRows = args.ElementAtOrDefault(4) as string[];
            var footerColumns = args.ElementAtOrDefault(5) as string[];
            var actorRegistry = serviceProvider.GetRequiredService<IActorRegistry>();
            var viewFactory = serviceProvider.GetRequiredService<IViewFactory>();
            var variableGridViewModel = new VariableGridViewModel(actorRegistry);
            var view = new VariableGridView(
                viewFactory,
                variableGridViewModel,
                rowCount,
                colCount,
                useCellName,
                headerColumns,
